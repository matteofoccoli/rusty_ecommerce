...
use rest_api::{configuration::get_settings, startup::run};

#[actix_web::main]
async fn main() -> Result<(), std::io::Error> {
    let configuration = get_settings().expect("Failed to read configuration fail");
...
fn create_connection_pool() -> Pool<ConnectionManager<PgConnection>> {
    let configuration = get_settings().expect("Failed to read configuration");
